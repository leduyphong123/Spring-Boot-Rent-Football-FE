...
function renderEdit(){
    $.ajax({
        ...
        success: (response) => {
            ...
            $("#registerTimeOpen").val(response.time_open);
            $("#registerTimeOpen").trigger("focus");

            $("#registerTimeClose").val(response.time_close);
            $("#registerTimeClose").trigger("focus");

            $("#registerDescribe").val(response.describes);
            $("#registerDescribe").trigger("focus");

            $("#registerDescription").val(response.description);
            $("#registerDescription").trigger("focus");

        
            let addressList = response.address.split(",");
            $("#registerCity").val(addressList[addressList.length-1]);
            $("#registerCity").trigger("focus");
            $("#registerDistrict").val(addressList[addressList.length-2]);
            $("#registerDistrict").trigger("focus");

            $("#registerTown").val(addressList[addressList.length-3]);
            $("#registerTown").trigger("focus");

            $("#registerStreet").val(addressList[addressList.length-4]);
            $("#registerStreet").trigger("focus");

            let addressHome ="";
            for(let i =0; i<addressList.length-4;i++){
                addressHome+=addressList[i];
            }
            $("#registerAddressHome").val(addressHome);
            $("#registerAddressHome").trigger("focus");
